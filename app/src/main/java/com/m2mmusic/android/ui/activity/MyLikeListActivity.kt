...
class MyLikeListActivity : BaseActivity(), MusicListAdapter.OnItemClickListener {

    private lateinit var binding: ActivityMusicListBinding
    private lateinit var musicList: ArrayList<Music>
    private lateinit var viewModel: MyLikeListActivityViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this)[MyLikeListActivityViewModel::class.java]

        musicList = intent.getParcelableArrayListExtra("like_list")
    ...
    private fun updateHeadUI() {
        binding.apply {
            ...
            Glide.with(this@MyLikeListActivity).load("${viewModel.profile!!.backgroundUrl}?param=400y400").apply(
                RequestOptions.bitmapTransform(BlurTransformation(this@MyLikeListActivity, 25, 2))
            ).into(binding.coverBackground)
            Glide.with(this@MyLikeListActivity).load("${viewModel.profile!!.backgroundUrl}?param=400y400")
                .into(binding.coverForeground)
            Glide.with(this@MyLikeListActivity).load("${viewModel.profile!!.avatarUrl}?param=40y40")
                .into(binding.creatorAvatar)
            binding.creatorName.text = viewModel.profile!!.nickname
            ...
            Glide.with(this@MyLikeListActivity)
                .asBitmap()
                .load(viewModel.profile!!.backgroundUrl)
                .into(simpleTarget)

            binding.songCount.text = "共 ${viewModel.musicList.size} 首"
        }
    }
        ...
        binding.bottomContainer.coverContainer.setOnClickListener {
            ...
            if (viewModel.currentPlayListEvent.playlist.isNullOrEmpty()) {
                ...
                viewModel.apply {
                    getProgress()
                    getDuration()
                    startActivity<MusicPlayActivity>(this@MyLikeListActivity) {
    ...
    companion object {
        private const val TAG = "MyLikeListActivity"
    }
